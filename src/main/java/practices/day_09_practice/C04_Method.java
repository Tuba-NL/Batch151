...
/*
    ...
    Her ogrencinin dogru cevap sayısını method olusturarak yazdırinız
...
*/
    ...
    public static void dogrucevapsayisi(char[][] answers, char[] keys) {
        ...
        for (int i = 0; i < answers.length; i++) {// 0`dan ogrenci sayisina kadar donguye aldik
            int sayac = 0;
            for (int j = 0; j < answers[i].length; j++) {// 0`dan ogrencinin cevap sayisina kadar donguye alacagiz
                if (answers[i][j] == keys[j]) {// ogrencinin cevabi cevap anahtarindakiyle esit mi kontrol ediyoruz
                    sayac++; // esitse sayaci bir artiralim
                }
